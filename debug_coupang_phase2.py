...
DEFAULT_URLS = [
    "https://m.coupang.com/",
    "https://m.coupang.com/nm/goldbox",
...
def run_requests_probe(
    ...
    for url in urls:
        ...
        print(f"[coupang_probe] requests url={url} result={meta_path}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for url in urls:
                ...
                print(f"[coupang_probe] browser url={url} result={meta_path}")
        finally:
            browser.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Coupang phase 2 probe")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path("scraper_debug") / f"coupang_phase2_{_timestamp()}"
